...
def RecogClass1vs1(fileName, camera, video_fps, video_frames, video_frames_PrintPercent, intervalTime):
    ...
    if not RecogComparisonMatrix:
        RecogComparisonMatrix = "No Similarity!"
    else:
        RecogComparisonMatrix = "Similarity Detected!"
            ...
            if intervalTime == 0:
                ...
                if Stu3_faceNum > 1 and Stu3_firstflag:
                    ...
                    if Recog_flag:
                        Recog_flag = 0
                        if len(Stu1_imgV) > 0:
                            imgV.append(Stu1_imgV)
                            FaceShapeV.append(Stu1_FaceShapeV)
                        if len(Stu2_imgV) > 0:
                            imgV.append(Stu2_imgV)
                            FaceShapeV.append(Stu2_FaceShapeV)
                        if len(Stu3_imgV) > 0:
                            imgV.append(Stu3_imgV)
                            FaceShapeV.append(Stu3_FaceShapeV)
                        if len(imgV) > 1:
                            RecogIsIllegal, RecogComparisonMatrix = PairWiseComparison(imgV, FaceShapeV)
                            if RecogIsIllegal:
                                for ii in range(len(imgV)):
                                    writedir = fileName + '_Recognized_Picture_%d.jpg' % ii
                                    cv2.imwrite(writedir, imgV[ii])
            ...
            if intervalTime == 0:
                ...
                if Recog_flag:
                    ...
                    if len(Stu3_imgV) > 0:
                        ...
                        FaceShapeV.append(Stu3_FaceShapeV)
                    if len(Stu4_imgV) > 0:
                        imgV.append(Stu4_imgV)
                        FaceShapeV.append(Stu4_FaceShapeV)
                    if len(imgV) > 1:
                        RecogIsIllegal, RecogComparisonMatrix = PairWiseComparison(imgV, FaceShapeV)
                        if RecogIsIllegal:
                            for ii in range(len(imgV)):
            ...
            if intervalTime == 0:
                ...
                if Recog_flag:
                    ...
                    if len(Stu4_imgV) > 0:
                        ...
                        FaceShapeV.append(Stu4_FaceShapeV)
                    if len(Stu5_imgV) > 0:
                        imgV.append(Stu5_imgV)
                        FaceShapeV.append(Stu5_FaceShapeV)
                    if len(imgV) > 1:
                        RecogIsIllegal, RecogComparisonMatrix = PairWiseComparison(imgV, FaceShapeV)
                        if RecogIsIllegal:
                            for ii in range(len(imgV)):
            ...
            if intervalTime == 0:
                ...
                if Recog_flag:
                    ...
                    if len(Stu5_imgV) > 0:
                        ...
                        FaceShapeV.append(Stu5_FaceShapeV)
                    if len(Stu6_imgV) > 0:
                        imgV.append(Stu6_imgV)
                        FaceShapeV.append(Stu6_FaceShapeV)
                    if len(imgV) > 1:
                        RecogIsIllegal, RecogComparisonMatrix = PairWiseComparison(imgV, FaceShapeV)
                        if RecogIsIllegal:
                            for ii in range(len(imgV)):
...
if __name__ == '__main__':
    ...
    # processVideo(VideoPath, GroupSize, intervalTime=5) 程序接口，VideoPath是待检测视频路径，GroupSize是小组课规模（取值是1，2，3，4，5，6），intervalTime是间隔检测时长（0表示全帧检测，正整数表示每隔几秒检测一帧。）
    #返回参数说明，总共返回四个参数，每个参数说明如下：
    # bIsItIllegal 返回当前被检测视频是否违规，0表示不违规，1表示违规。
    # bCaseReason 返回当前被检测视频违规的具体原因。
    # bRecogComparisonMatrix 返回当前被检测视频中每个学生界面是否含有相似的人脸。
    # bStu_FaceShowRatio 返回当前被检测视频中每个学生界面的人脸出现占比。
    bIsItIllegal, bCaseReason, bRecogComparisonMatrix, bStu_FaceShowRatio = processVideo(VideoPath, 6, 10)
